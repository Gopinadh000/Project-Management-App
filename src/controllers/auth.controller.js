...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (companies.length > 0) {
      return ReE(res, {
        message:
          "Company ID already exists. Please use a different company ID.",
      });
    }
    ...
    await connection.query(
      "INSERT INTO users_details_info(user_id , first_name , last_name)  VALUES (? , ? , ?)",
...
export const loginUser = async (req, res) => {
  ...
  const [pwRows] = await db.query(
    "SELECT hash_password FROM users_passwords WHERE user_id = ?",
    [user.id]
  );
  if (pwRows.length === 0) {
