...
const TasksPage = ({ TabsData, border=true }: any) => {
  ...
  return (
    <div>
      ...
      <div className={`h-10 mt-2 flex items-center justify-between px-1 py-6 ${border && 'border'}`}>
        <div>
         <TasksTabs  TabsData={TabsData} />
        </div>
        <div>
          <Button onClick={()=> setOpenModal(true)} variant="contained">Add Task</Button>
        </div>
      </div>
      <div className="mt-10 ">
        {
      ...
      <Modal size="md" modalType="side" open={openModal} onClose={() => setOpenModal(false)}>Hello </Modal>
      {/* <TasksModal  openModal={openModal}   onClose={() => setOpenModal(false)} /> */}
    </div>
  );
